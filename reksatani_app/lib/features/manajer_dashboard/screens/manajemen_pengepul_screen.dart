...
class _ManajemenPengepulScreenState extends State<ManajemenPengepulScreen> {
  ...
  DateTime? _getWaktuSyncTerakhir(String pengepulId) {
    ...
    for (var t in trxAgen) {
      if (terbaru == null || t.createdAt.isAfter(terbaru)) terbaru = t.createdAt;
      if (t.waktuDisinkron != null) {
        if (terbaru == null || t.waktuDisinkron!.isAfter(terbaru)) terbaru = t.waktuDisinkron;
      }
    }
    return terbaru;
  }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _PengepulFormSheet(
        ...
        onSimpan: (username, password, sisaUangJalan) async {
          ...
          bool sukses = false;
          if (dataLama == null) {
            sukses = await _controller.tambahPengepul(username: username, password: password, sisaUangJalan: sisaUangJalan);
          } else {
            sukses = await _controller.editPengepul(id: dataLama['_id'], username: username, password: password, sisaUangJalan: sisaUangJalan);
          }
          if (!sukses && mounted) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(width: 64, height: 64, decoration: BoxDecoration(color: AppTheme.hijauSoft, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppTheme.hijauMuda.withOpacity(0.3))), child: Center(child: Text(item['username']?.toString().substring(0,1).toUpperCase() ?? '👤', style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: AppTheme.hijauTua)))),
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Text(online ? 'Sedang Online' : _timeAgo(lastSync), style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: online ? AppTheme.hijauTua : AppTheme.textSecond)),
                                ],
                              ),
                            ),
                          ],
              ...
              Container(
                ...
                decoration: BoxDecoration(gradient: AppTheme.headerGradient, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: AppTheme.hijauMuda.withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 8))]),
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(8)), child: Text((item['role'] ?? 'pengepul').toString().toUpperCase(), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Colors.white))),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Text(_fmtRupiah((item['sisa_uang_jalan'] ?? 0).toDouble()), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: -1.0)),
                      ],
                    ),
                  ],
                ),
              ...
              if (history.isEmpty)
                Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: AppTheme.bgPage, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppTheme.border)), child: const Center(child: Text('Belum ada transaksi lapangan.', style: TextStyle(fontSize: 13, color: AppTheme.textSecond, fontWeight: FontWeight.w600))))
              else
                ...history.map((t) => _TrxDetailRow(trx: t)),
              const SizedBox(height: 32),
              SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: AppTheme.bgPage, foregroundColor: AppTheme.textPrimary, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: const BorderSide(color: AppTheme.border))), child: const Text('Tutup Panel', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)))),
            ],
          ),
        );
      },
...
class _TrxDetailRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppTheme.border), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 8, offset: const Offset(0, 4))]),
      child: Row(
        children: [
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppTheme.bgPage, borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.receipt_long_rounded, color: AppTheme.textSecond, size: 20)),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${trx.namaKomoditas} · ${trx.berat.toInt()} kg', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14, color: AppTheme.textPrimary)),
                const SizedBox(height: 4),
                Text('Mitra: ${trx.namaPetani} · ${_fmtDate(trx.createdAt)}', style: const TextStyle(fontSize: 12, color: AppTheme.textSecond, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
          Text(_fmtRupiah(trx.totalBayar), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15, color: AppTheme.hijauTua, letterSpacing: -0.5)),
        ],
      ),
    );
  }
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(widget.dataLama == null ? 'Daftarkan Agen Baru' : 'Edit Akun Agen', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: AppTheme.textPrimary, letterSpacing: -0.5)),
