        ...
        children: [
            ...
            children: [
              Container(
                ...
                child: CircleAvatar(
                  ...
                  child: Text(user.username.substring(0, 1).toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 18)),
                ),
              ),
              ...
              _IkonBtnHeader(icon: Icons.logout_rounded, onTap: onLogout, color: Colors.red.shade50, iconColor: AppTheme.merah),
            ],
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(24),
              boxShadow: [BoxShadow(color: AppTheme.hijauMuda.withOpacity(0.35), blurRadius: 25, offset: const Offset(0, 12))],
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                const SizedBox(height: 16),
                Text(_fmtRupiah(totalNilai), style: const TextStyle(color: Colors.white, fontSize: 34, fontWeight: FontWeight.w900, letterSpacing: -1.0)),
...
class _BentoSyncRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _BentoStatCard(label: 'Menunggu Sync', value: '$jumlahPending Transaksi', icon: Icons.cloud_off_rounded, color: const Color(0xFFF59E0B)),
        const SizedBox(width: 14),
        _BentoStatCard(label: 'Sudah Sinkron', value: '$jumlahSynced Transaksi', icon: Icons.cloud_done_rounded, color: AppTheme.hijauMuda),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppTheme.border.withOpacity(0.5)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 12, offset: const Offset(0, 4))]),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color.withOpacity(0.12), shape: BoxShape.circle), child: Icon(icon, color: color, size: 20)),
...
class _KomoditasRowModern extends StatelessWidget {
  ...
  final bool hasGrade;
  const _KomoditasRowModern({required this.namaKomoditas, required this.jumlahGrade, required this.hargaMaks, required this.hasGrade});
  @override
  Widget build(BuildContext context) {
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: AppTheme.bgPage, borderRadius: BorderRadius.circular(6)), child: Text('$jumlahGrade Kualitas Grade', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppTheme.textSecond))),
              ],
            ),
          ),
          if (hasGrade)
...
class _KasAgenCardModernState extends State<_KasAgenCardModern> {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: AppTheme.border.withOpacity(0.5)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.025), blurRadius: 15, offset: const Offset(0, 5))]),
        ...
        children: [
          InkWell(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  Container(width: 46, height: 46, decoration: BoxDecoration(color: AppTheme.bgPage, borderRadius: BorderRadius.circular(14)), child: const Center(child: Text('👤', style: TextStyle(fontSize: 18)))),
                  const SizedBox(width: 14),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Row(
                          children: [
                            Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: _isOnline ? AppTheme.hijauMuda : Colors.grey.shade400)),
                            const SizedBox(width: 6),
                            Text(_isOnline ? 'Aktif Lapangan' : _timeAgo, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _isOnline ? AppTheme.hijauTua : AppTheme.textSecond)),
                          ],
                        ),
                      ],
                    ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      const Text('5 Aktivitas Terakhir Agen:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppTheme.textSecond)),
                    ],
                  ),
                  const SizedBox(height: 12),
                  if (widget.history.isEmpty)
                    const Padding(padding: EdgeInsets.symmetric(vertical: 8), child: Text('Belum ada rekaman transaksi dari agen ini.', style: TextStyle(fontSize: 12, color: AppTheme.textHint, fontWeight: FontWeight.w500)))
                  else
...
class _IkonBtnHeader extends StatelessWidget {
  ...
  const _IkonBtnHeader({required this.icon, required this.onTap, required this.color, required this.iconColor});
  @override
...
class _EmptyCardBento extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppTheme.border.withOpacity(0.6))),
        child: Center(
          child: Column(
            children: [
